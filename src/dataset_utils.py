...
# we get 80% of each decade for training and 20% for testing
def get_dataset_flushed_splitted(train_size=0.8, validate_size=0.1, columns_to_drop=['track', 'artist', 'uri'], datasets_to_test=["00", "10"], dataset_names_to_join=["60", "70", "80" , "90"]):
    """
    For each dataset of each decade, get random 20% and add it to a dataset
    """
    # Llegim dades
...
def get_datasets(columns_to_drop=['track', 'artist', 'uri'], datasets_to_test=["00", "10"], dataset_names_to_join=["60", "70", "80" , "90"]):
    # Llegim dades
    total_index = ["60", "70", "80" , "90", "00", "10"]
    total_index_full = ["1960", "1970", "1980" , "1990", "2000", "2010"]
...
def get_full_dataset(columns_to_drop=['track', 'artist', 'uri']):
    ...
    for i in range(len(total_index)):
        ...
        dataset.insert(0, 'decade', total_index_full[i])
        dataset.reset_index(drop=True, inplace=True)
        temp_ds_train.append(dataset)

    # for i in range(len(temp_ds_train)):
    #     temp_ds_train[i] = temp_ds_train[i].reset_index(drop=True)
